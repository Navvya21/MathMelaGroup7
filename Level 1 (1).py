...
questions = [v for v in data1[0].values()]
answers_choice = [v for v in data1[1].values()]
...
def gen():
    ...
    while(len(indexes) < 5):
        ...
        if x in indexes:
            ...
            indexes.append(x)
def open_file(): 
    file = askopenfile(mode ='r', filetypes =[('Python Files', 'Level 2.py')])
...
def showresult(score):
    ...
    if score == 25:
        ...
        btn = Button(root, text = 'Level 2 !', bd = '5',command = lambda: open_file())
        btn.pack(side = 'top') 
...
lblInstruction = Label(
    root,
    text = "Level 1: Easy: Click Start Once You Are ready",
    background = "#ffffff",
    font = ("Consolas",14),
    justify = "center",
)
