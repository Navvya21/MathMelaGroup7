...
questions = [v for v in data2[0].values()]
answers_choice = [v for v in data2[1].values()]
...
def gen():
    ...
    while(len(indexes) < 5):
        ...
        if x in indexes:
            ...
            indexes.append(x)
def open_file(): 
    file = askopenfile(mode ='r', filetypes =[('Python Files', 'Level 3.py')])
...
def showresult(score):
    ...
    if score == 25:
        ...
        labelresulttext.configure(text = "Awesome! Your score is 25!Proceed to \n Level 3")
        btn = Button(root, text = 'Level 3 !', bd = '5',command = lambda: open_file())
        btn.pack(side = 'top')
    elif (score == 20):
        img = PhotoImage(file="great.png")
        labelimage.configure(image=img)
        ...
        labelimage.configure(image=img)
        labelimage.image = img
        labelresulttext.configure(text= "Not Bad!!Your score is 15!Proceed to \n Level 3")
        btn = Button(root, text = 'Level 3 !', bd = '5',command = lambda: open_file())
        btn.pack(side = 'top')
    elif (score == 10):
        ...
        labelimage.configure(image=img)
        labelimage.image = img
        labelresulttext.configure(text= "Work Harder!!Your score is 10!\n You don't qualify for Level 3")
        btn = Button(root, text = 'Exit !', bd = '5',command = root.destroy)
        btn.pack(side = 'top')
    elif (score == 5):
        img = PhotoImage(file="bad.png")
        labelimage.configure(image=img)
...
lblInstruction = Label(
    root,
    text = "Level 2: Intermediate: Click Start Once You Are ready",
    background = "#ffffff",
    font = ("Consolas",14),
    justify = "center",
)
